...
class Glowna:
	...
	def oHTMLowany(self):
		...
		for nr_dzialu, nazwa_dzialu, opis_dzialu in self.dzialy():
			dzialy += self.szablon_dzialu.format(nr=nr_dzialu, nazwa=six.ensure_str(nazwa_dzialu), opis=six.ensure_str(opis_dzialu))
		najnowsze = ""
		for nr_watku, tresc, autor, data_wyslania, nr_wpisu in self.najnowsze():
			...
			najnowsze += self.szablon_wpisu.format(autor=autor, czas_iso=czas_iso, czas_ladnie=czas_ladnie, tresc=six.ensure_str(tresc), nr_watku=nr_watku, nr_wpisu=nr_wpisu)
